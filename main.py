...
def create_app() -> Client:
    return Client(name="startlove_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=50, in_memory=True)
...
def main():
    ...
    try:
        ...
        idle()
    except KeyboardInterrupt: logger.warning("Bot stopped manually")
    except RPCError as e: logger.error(f"Telegram RPC error: {e}")
    except Exception as e: logger.exception(f"Fatal error: {e}")
    finally:
        try: app.stop(); logger.info("Bot stopped gracefully")
        except: pass
